...
export default function GamePage() {
  ...
  const hasTriggeredInitialSpin = useRef(false);
  ...
  // Trigger initial spin animation on load (only once)
  useEffect(() => {
    if (!gameEngine.loading && gameEngine.shouldShowInitialSpin && !hasTriggeredInitialSpin.current) {
      hasTriggeredInitialSpin.current = true;
      setIsSpinning(true);
      setTimeout(() => {
        setIsSpinning(false);
      }, 800);
    }
  }, [gameEngine.loading, gameEngine.shouldShowInitialSpin]);
        ...
        <div className="container">
          ...
          <div className="mb-12">
            <ResultCard 
              ...
              ranking={gameResult.finalRanking} 
              showConfetti={false}
              selectedCountries={gameResult.selectedCountries}
            />
          </div>
          ...
          <Card className="card-elevated p-8 md:p-12 mb-8 max-w-4xl mx-auto bg-gradient-to-br from-orange-50 to-pink-50 border-2 border-orange-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(gameResult.selectedCountries).map(([category, countryId]: any) => {
                ...
                const categoryScore = gameResult.categoryScores?.[category as Category];
                return (
                  <div
                    key={category}
                  ...
                  >
                    ...
                    <p className="font-bold text-foreground text-sm">{getCountryById(gameResult.selectedCountries[category as Category])?.name || "Desconhecido"}</p>
                    {categoryScore !== undefined && (
                      <p className="text-xs text-orange-600 mt-1">{categoryScore.toFixed(1)}</p>
                    )}
                  </div>
                );
              })}
