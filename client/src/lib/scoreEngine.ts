...
export function calculateFinalScore(selectedCountries: Record<Category, string>): {
  ...
  for (const category of categoryConfigs) {
    ...
    if (countryId) {
      ...
      if (scores) {
        // Usar o ID da categoria como chave
        const categoryKey = category.id as keyof typeof scores;
...
export function getRankingLabel(score: number): string {
  ...
  const tier = RANKING_TIERS.find((t) => normalizedScore >= t.minScore && normalizedScore <= t.maxScore);
  // Se não encontrar, retornar a tier padrão (Desastre Absoluto)
  if (!tier) {
    const defaultTier = RANKING_TIERS[RANKING_TIERS.length - 1];
    return `${defaultTier.label} (${defaultTier.description})`;
  }
